...
var normName = (v, d) => typeof v === "string" && v.trim().length > 0 ? v : d;
...
var DEFAULT_IDLE_RECLAIM_SEC = 30;
...
function apply(ctx, config) {
  ...
  const configIdleReclaimMs = typeof config?.idleReclaimMs === "number" && config.idleReclaimMs > 0 ? config.idleReclaimMs : null;
  ...
  void Promise.all([
    ...
    const Config = z.object({
      ...
      idleReclaimSec: z.number().step(1).min(1).max(120).default(DEFAULT_IDLE_RECLAIM_SEC)
    });
    installSettingsSection(ctx, settingsNamespace("ssh-hub"), Config, {}, {
      setSource: (source) => {
        ...
        if (configIdleReclaimMs === null) {
          const v = source() ?? {};
          const sec = typeof v.idleReclaimSec === "number" && Number.isFinite(v.idleReclaimSec) && v.idleReclaimSec >= 1 && v.idleReclaimSec <= 120 ? v.idleReclaimSec : DEFAULT_IDLE_RECLAIM_SEC;
          currentIdleReclaimMs = sec * 6e4;
  ...
  let workspaceState = defaultCollection();
  const workspaceClients = /* @__PURE__ */ new Set();
  function sanitizeWorkspaces(input) {
    ...
    const alive = (id) => typeof id === "string" && registry.get(id) !== void 0;
    const items = collection.items.map((it) => {
      ...
      if (it.kind === "workspace") {
        const members = it.members.filter((m) => alive(m.sessionId));
  ...
  function clearSessionFromWorkspaces(id) {
    const items = workspaceState.items.map((it) => {
      if (it.kind === "tab" && it.sessionId === id) return null;
      if (it.kind === "workspace") {
        const members = it.members.filter((m) => m.sessionId !== id);
        const sizes = it.sizes.slice(0, members.length);
        return members.length === 0 ? null : { ...it, members, sizes };
      }
      return it;
    }).filter((it) => it !== null);
    if (items.length !== workspaceState.items.length || items.some((it, i) => it !== workspaceState.items[i])) {
