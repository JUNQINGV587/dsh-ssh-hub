...
function normalizeTab(input) {
  ...
  const tree = input?.tree !== null && typeof input?.tree === "object" && input.tree.kind === "split" ? newBlock(null) : normalizeTree(input?.tree);
  return { name, tree };
}
function normalizeWorkspace(input) {
  ...
  const rawTabs = Array.isArray(input?.tabs) && input.tabs.length > 0 ? input.tabs : [void 0];
...
function emptySessionFromAll(collection, sessionId) {
  let changed = false;
...
function apply(ctx, config) {
  ...
  function sanitizeWorkspaces(input) {
    ...
    const walk = (node) => {
      if (node.kind === "block") {
        const sessionId = node.sessionId !== null && registry.get(node.sessionId) === void 0 ? null : node.sessionId;
        return { kind: "block", sessionId };
      }
      return { ...node, children: (node.children ?? []).map(walk) };
    };
    return {
      workspaces: collection.workspaces.map((w) => ({
        ...w,
        tabs: w.tabs.map((t) => ({ ...t, tree: walk(t.tree) }))
      })),
