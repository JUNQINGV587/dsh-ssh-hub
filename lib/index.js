...
function apply(ctx, config) {
  ...
  let gridState = { template: "single", tiles: [null] };
  const gridClients = /* @__PURE__ */ new Set();
  function sanitizeGrid(input) {
    if (input?.template !== void 0 && !TEMPLATES.includes(input.template)) return null;
    ...
    const tiles = input.tiles.map((t) => typeof t === "string" ? t : null);
    for (let i = 0; i < tiles.length; i++) {
      if (tiles[i] !== null && registry.get(tiles[i]) === void 0) tiles[i] = null;
    }
