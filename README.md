...
- 🌳 **Split-tree workspace** — blocks hold one session each, split recursively in four directions from a block's title bar; dividers drag to resize; drag a block onto another's centre to swap, or onto an edge to open a new pane there (red=left, green=right, cyan=top, blue=bottom preview). How many panes fit is decided by the window size
...
- 🎹 **Configurable shortcuts** — 设置 → 插件 → 插件配置 → DSH-SSH-HUB: set the toggle-window and maximize bindings (validated, DSH-conflict warning, applied immediately)
