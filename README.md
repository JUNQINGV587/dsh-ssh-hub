...
- 🌳 **Workspaces of tabs of split trees** — a workspace is a named/iconed/colored layout template holding tabs; each tab is a split tree of blocks. Switch workspaces from the switcher (new from empty template or copy-current), switch tabs in the tab bar (Alt+1-9, F2 renames), split recursively in four directions from a block's title bar, drag dividers, drag a block onto another's centre to swap or onto an edge to open a new pane (red=left, green=right, cyan=top, blue=bottom preview)
