...
- 🌳 **标签页 × 布局树**：每个标签页是一棵 Wave 式 flexbox 布局树（行列组合的块，自动向右排布、满 5 换行）。标签栏切换标签页（Alt+1-9，F2 重命名，Alt+t 新建）；把块拖到另一块上有全部 7 种 Wave 落点（同行前后插队 / 对向内外层新建层级 / 中央互换，绿色落点提示）；拖块边距调整大小
